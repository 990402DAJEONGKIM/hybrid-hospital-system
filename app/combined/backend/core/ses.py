...
def _ses_send(to_email: str, subject: str, body: str) -> bool:
    ...
    if not SES_FROM_EMAIL:
        logger.warning("SES_FROM_EMAIL 미설정 — 알림 생략: %s", to_email)
        return False
    try:
        import boto3
        ...
        logger.info("SES 발송 완료: %s → %s", subject[:30], to_email)
        return True
    except ImportError:
        logger.warning("boto3 미설치 — SES 알림 생략 (%s)", to_email)
        return False
    except Exception as exc:
        logger.error("SES 발송 실패 (%s): %s", to_email, exc)
        return False
