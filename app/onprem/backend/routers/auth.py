...
def login(
    ...
    if not user:
        record_login_history(db, "fail", email=body.email, ip_address=ip, user_agent=ua)
        db.commit()
    ...
    if user.locked_until and user.locked_until > now:
        record_login_history(db, "locked", email=body.email, user_id=user.user_id, ip_address=ip, user_agent=ua)
        db.commit()
    ...
    if not user.is_active:
        record_login_history(db, "fail", email=body.email, user_id=user.user_id, ip_address=ip, user_agent=ua)
        db.commit()
    ...
    if not verify_password(body.password, user.password_hash):
        ...
        record_login_history(db, result, email=body.email, user_id=user.user_id, ip_address=ip, user_agent=ua)
        db.commit()
        raise HTTPException(status_code=401, detail="이메일 또는 비밀번호가 올바르지 않습니다.")
    ...
    record_login_history(db, "success", email=body.email, user_id=user.user_id, ip_address=ip, user_agent=ua)
...
def logout(
    ...
    if refresh_token:
        ...
        if session:
            ...
            ip = request.client.host if request.client else None
            record_audit(db, "LOGOUT", "204", user_id=session.user_id, source_ip=ip)
            db.commit()
...
def change_password(
    ...
    ip = request.client.host if request.client else None
    record_audit(db, "PASSWORD_CHANGE", "204", user_id=user.user_id, source_ip=ip)
    db.commit()
