...
echo
echo "=== 4. force Cloudflare DNS back to AWS ==="
gcloud compute ssh "$PROXY_INSTANCE" --project "$PROJECT" --zone "$ZONE" --command '
set -euo pipefail
...
echo
echo "=== 6. wait GCP DR MIG stable at 0 ==="
for i in {1..30}; do
...
echo
echo "=== 7. reset monitor state=aws and start monitor ==="
gcloud compute ssh "$PROXY_INSTANCE" --project "$PROJECT" --zone "$ZONE" --command '
set -e
