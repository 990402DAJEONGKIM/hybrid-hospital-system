...
def _collect_month(cf_url: str, api_key: str, year: str, month: str, partial: bool = False) -> dict:
    today = date.today()
...
def lambda_handler(event, context):
    ...
    if event.get("year") and event.get("month"):
        ...
        return _collect_month(cf_url, api_key, year, month, partial=is_partial)

    # 기본: 전월(완료) + 당월(집계 중) 수집
    prev_month, cur_month = _get_months()
    prev_result = _collect_month(cf_url, api_key, *prev_month, partial=False)
    cur_result  = _collect_month(cf_url, api_key, *cur_month,  partial=True)
