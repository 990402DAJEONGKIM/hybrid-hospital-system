...
def _build_prompt(agg, vuln):
    ...
    return (
        ...
        "형식 규칙:\n"
        "- 마크다운 표(|---|)를 절대 사용하지 마세요. 모든 내용은 문장으로 서술하세요.\n"
        "- 규칙ID, CVE 번호를 나열하지 말고, 무슨 일이 있었고 왜 위험한지를 자연스러운 문장으로 풀어 쓰세요.\n"
        "- 숫자(건수, CVSS 점수)는 문장 안에 자연스럽게 포함하세요.\n\n"
        "아래 순서로 작성하세요:\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        "🏥 일일 보안 보고서\n"
        "■ 1. 종합 위험도 평가 — 정상/주의/경계/심각 중 하나를 밝히고, 그 근거를 두세 문장으로 서술\n"
        "■ 2. 보안 이벤트 현황 — 즉시 조치가 필요한 사안, 주의가 필요한 사안, 모니터링 대상 순으로 각각 한 단락씩 서술. "
        "가장 심각한 사건 1~2개를 중심으로 무슨 일이 일어났는지 스토리로 설명\n"
        "■ 3. 취약점 현황 — 전체 건수를 한 문장으로 요약한 뒤, 가장 위험한 자산 2~3개를 골라 "
        "무슨 패키지에 어떤 문제가 있고 왜 위험한지 단락으로 설명. CVE 번호를 나열하지 말 것. "
        "단락 마지막에는 '지금 바로 적용해야 할 업데이트' 5가지를 우선순위 순으로 정리하세요. "
        "각 항목은 '패키지명을 몇 버전 이상으로 올려야 한다'는 형태로 명확하게 쓰고, "
        "fix 필드 값(예: Package less than X)을 참고해 목표 버전을 명시하세요.\n"
        "■ 4. 종합 의견 및 권고사항 — 우선순위 순으로 무엇을 해야 하는지 단락으로 서술\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
