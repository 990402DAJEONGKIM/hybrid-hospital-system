...
def update_aws_secret(secret_id: str, new_password: str) -> None:
    ...
    sm.put_secret_value(SecretId=secret_id, SecretString=json.dumps({"password": new_password}))
...
def change_rds_password(username: str, new_password: str, rds_admin_password: str) -> None:
    conn = psycopg2.connect(
        host=RDS_ENDPOINT, port=5432, user="hospital_user",
        password=rds_admin_password, dbname="hospital",
        sslmode="require", connect_timeout=10,
    )
    try:
        conn.autocommit = True
...
def get_rds_admin_password() -> str:
    sm = boto3.client("secretsmanager", region_name=AWS_REGION)
    secrets_list = sm.list_secrets(Filters=[{"Key": "name", "Values": ["rds!cluster"]}])
